...
def download_odds_for_races(
    ...
    for i, info in enumerate(race_infos, 1):
        ...
        if i % CHECKPOINT == 0 or i == total:
            logger.info(
                ...
                i, total, success, empty,
            )

        try:
            odds = fetch_odds(stadium_id, race_date, race_no)
        except Exception as exc:
        ...
        if not odds:
            logger.debug("Empty odds for %s", race_id)
            empty += 1
            continue
