...
> ⚠️ **凍結中（2026-04-24〜）**
>
> 3 エージェント独立レビューの結果、本ループは以下の理由で凍結推奨となった:
> - BET_RULE_REVIEW §30-32 で「フィルタ探索では out-of-sample 黒字化不能」と判定済み
>   （12ヶ月通算 ROI -13.4%、破局月 -65〜-80%）
> - 単月 ROI の偶発変動で ΔROI +50pp は容易に達成され、現行採択ロジックでは
>   破局月を baseline にすると必ず誤採択される
> - 1 着識別能力がランダム（モデル側ボトルネック）であり、フィルタ調整は堂々巡り
>
> **先に `/model-loop`（フェーズ6）でモデル側を改善すること。**
> 再開条件: `/model-loop` で通算 ROI ≥ 0% を達成し、本手順書の採択ロジックを
> 単月評価 → 多月ホールドアウト評価に再設計してから。
>
> なお 2026-04-24 に以下の最小パッチを実装済み:
> - C1: `run_gate_check.py` に `--label` 引数追加（JSON 衝突解消）
> - C3/C4/C5: `claude_fix.yml` を YAML 読み取り専用化（CODEOWNERS 迂回遮断、
>   exclude_courses 空文字クラッシュ解消、YAML 編集が実ランに届かない問題解消）
> - これに伴い本手順書 Step 8 の CLI 引数渡し方式は無効化されている。
>   再開時は「YAML を書き換える → workflow をキックする」だけのシンプルな
>   流れに作り直す必要がある。
