...
**最終更新**: 2026-04-27（Day 1 セッション 2 完了 / 8 場 = 96 races 予想実行済）
**前提コミット**: `6274747 feat(llm-predict): P4-β Day 1 S2 - 2026-01-05 多摩川・びわこ・住之江・尼崎 4 場予想実行`
...
|---|---|---|---|---|---|---|---|---|---|
| 1 | 2026-01-05 | 16 | 🟡 8/16 場 完了 (S1+S2) | 192 | 16 (S1+S2 部分) | 未集計 | 未集計 | 未集計 | 残り 8 場で S3-S4 |
...
| S3 | 鳴門・丸亀・徳山・下関 | 🟡 未着手 | 48 | — | **このセッションでやる** |
...
## 今セッションでやること（S3: 鳴門・丸亀・徳山・下関）

### 1. 状況確認

```bash
# S1+S2 で生成済 = 96 件 (8 場 × 12R) の予想 + 192 件の _pre が並ぶ
ls artifacts/predictions/2026-01-05/*.json | grep -v _pre | grep -v index | wc -l   # 96 + 1 index = 97 を確認
...
```bash
grep -l "## ▼ 直前情報" artifacts/race_cards/2026-01-05/14_*.md  # 鳴門 12 件出れば OK
grep -l "## ▼ 直前情報" artifacts/race_cards/2026-01-05/15_*.md  # 丸亀
grep -l "## ▼ 直前情報" artifacts/race_cards/2026-01-05/18_*.md  # 徳山
grep -l "## ▼ 直前情報" artifacts/race_cards/2026-01-05/19_*.md  # 下関
```
...
```
/predict 2026-01-05 鳴門
/predict 2026-01-05 丸亀
/predict 2026-01-05 徳山
/predict 2026-01-05 下関
```
...
144（このセッション完了時 = S1 48 + S2 48 + S3 48）/ 192（全場処理完了時）。
...
> **注**: S2 末尾で 96 件分の `index.json` は既に生成済（スキーマ検証パス確認用）。
> S3 末尾でも追加検証として `build_predictions_index.py 2026-01-05` を再実行 OK
> （144 件 valid / 0 invalid を確認）。最終評価は S4 完了後の 192 件全件で行う。
...
- **A1×3-4 充実メンバーの予選特選**: 1号艇単勝 1.0-1.2x で 3連単オッズが薄く EV<0.5、原則 skip
- **6号艇A1/A2 の異常好調シグナル**（S2 で 3 例採用）:
...
- S2 セッション末尾の index 検証: `artifacts/predictions/2026-01-05/index.json` (96 valid / 0 invalid, bet 16 / skip 80)
