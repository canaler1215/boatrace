...
- **フェーズ B-3「単勝市場効率分析」Step 1 + 12 ヶ月 DL 完了、Step 2 集計着手可**（2026-04-27）
  ...
  - **次は Step 2 集計**: `run_market_efficiency.py --bet-type win` 拡張で lift / `ev_all_buy` /
    bootstrap CI を 12 ヶ月集計、採用判定 → Step 3（サブセグメント）or B-3 撤退
  - 次セッション着手用: [NEXT_SESSION_PROMPT.md](NEXT_SESSION_PROMPT.md) を貼り付け（B 用、Step 2 集計）
  - 関連プロンプト（B 完了後）: [NEXT_SESSION_PROMPT_A.md](NEXT_SESSION_PROMPT_A.md)（複勝 DL 実装）
    / [NEXT_SESSION_PROMPT_C.md](NEXT_SESSION_PROMPT_C.md)（2 連単 / 2 連複 / 拡連複 DL 実装）
  - 詳細計画: [NEXT_PHASE_B3_PLAN.md](NEXT_PHASE_B3_PLAN.md) / 結果: [MARKET_EFFICIENCY_WIN_RESULTS.md](MARKET_EFFICIENCY_WIN_RESULTS.md)
...
- **P4-α 完了 → P4-β（場拡大）へ移行**（2026-04-27）
  - **P4-α 結果**: 12 race-days × 桐生 1 場 = 596 bets / hit_rate 7.05% / **ROI -2.78%**
    - 月次: 9 月 +32.3% / 10 月 -9.3% / 11 月 -3.2% / 12 月 -32.2%
    - bootstrap 95% CI: [-33.6%, +30.7%]、判定 = fail
  - **しかし「fail 判定は統計的に不当」と判明**（2026-04-27 サンプルサイズ分析）:
    - bet 単位の SD = 401 円（mean -2.79 円の 144 倍 = 極端な右裾分布）
    - 真の ROI = +10% を 0% から区別するには **9,979 bets ≈ 200 race-days** 必要
    - P(真の ROI > 0%) = 41.2%、P(真の ROI > +10%) = 22.2%（不確定領域）
  - **深掘りで確認された signal**:
    - primary_axis 1-2着集合一致時 ROI **+118.5%** / 不一致時 -46.6%（n=158/438）
    - actual 1着 が axis に含まれる時 ROI +57.6% / 含まれない時 -84.7%（n=242/220）
    - pick_pos == 1 (top conf) 単体で ROI +7.3%、EV ≥ 1.3 で ROI +11.5%
    - ただし**全てのフィルタが 9 月偏重**（10/11/12 月では大半マイナス）
  - **P4-β 設計**: predict.md は不変、**場拡大 (1 場固定 → 全開催場)** で 1 race-day あたりの
    bets を ~50 → ~700-1000 へ。同じ prompt が大規模サンプルで signal を持つかを測定問題として解く
  - **OOS 期間**: 2026-01〜2026-04（P4-α は 2025-09〜2025-12 = 4 ヶ月、OOS 用に未着手 4 ヶ月）
  - **目標**: 12 race-days で ~8,000-15,000 bets、CI 半幅 ±15pp 未満を目指す
  - **Day 1 prep 完了**: 2026-01-05、16 場開催、192 races、`artifacts/race_cards/2026-01-05/` 配置済み
  - **撤退ライン**（P4-α より厳格化）: 通算 ROI < -15% かつ 月次 ROI < -10% が 3 ヶ月以上
    かつ bootstrap CI 上限 < +5%、すべて満たせば B-3 単勝市場効率 Step 2 へ
  - **厳守事項**: predict.md / predict_llm/ / evaluate_predictions.py / eval_summary.py すべて
    書き換え禁止、後付けフィルタ追加禁止、場サブセット選択禁止（全開催場を必ず処理）
  - **詳細計画**: [NEXT_PHASE_P4B_PLAN.md](NEXT_PHASE_P4B_PLAN.md) / 分析結果: [NEXT_SESSION_PROMPT_P4.md](NEXT_SESSION_PROMPT_P4.md)（P4-α 履歴）
