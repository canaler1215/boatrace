...
## 現行の運用方針（2026-04-28 時点）

### 🛑 全フェーズ凍結確定（2026-04-28、Q-B 合意）— **(P-v) ハイブリッド採用**

ML 系（フェーズ 3 / 6）、B 系列（B-1 trifecta / B-3 win / 拡張 A 複勝 / P-v condition）、
P 系列（P1〜P4-β LLM 予想）の **6 系統独立検証が全て採用基準未達**で終了。

| 系統 | 検証規模 | 結果 |
|---|---|---|
| B-1 trifecta | 全期間 + 24 場 + odds_band 2 軸 | 0/62 セグメント、最善 ev=0.98 |
| B-3 win | 12 ヶ月 / 50,483 races | 0/82 セグメント、最善 ev=0.964 |
| 拡張 A 複勝 | 12 ヶ月 + payout 補正 | 0/81 セグメント、最善 ev=0.93 |
| P-v condition | wind × weather | 0/16 セグメント、最善 ev=0.95 |
| **P4-α LLM** | **596 bets / confidence × 4 帯** | **0/4 帯、最善 ROI +1.52% (CI 下限 -34.7%)** |
| Model-loop | 16 trials + lambdarank seed×3 | T16 で seed ノイズ床 17pp 確認 |

**採用方針 = (P-v) ハイブリッド**:
- P1〜P3.5 のスクリプト群 (`/prep-races` / `/predict` / `/eval-predictions` / `/eval-summary`) は **保持**（撤退コストの方が高い、別問題定義での再起動余地）
- ROI 改善目標は **捨てる**。自動購入は実施しない
- `/predict` を月数回任意で回すのは可。ただし「production 化への第一歩」と捉えない
- 後付けフィルタ追加 / prompt 改善 / 場限定運用への移行は **禁止**（フェーズ 3〜6 の教訓）
- 構造変更は新しい問題定義（別 KPI / 別市場 / 別データソース）が出てきた時のみ検討

**GitHub Actions 全定期実行停止済み**: `auto_backtest_loop` / `build_racer_st_stats` /
`fetch_results` / `quarterly_walkforward` / `collect` / `predict` / `refresh_ev` の
schedule + repository_dispatch トリガーをコメントアウト。manual dispatch のみ残置。
cron-job.org 側の `collect-trigger` / `predict-trigger` / `refresh-ev-trigger` ジョブも別途無効化要。

**手動でレース予想したくなった時の操作**: 本ファイル末尾「手動レース予想の手順 (P-v 凍結後)」セクション参照。

詳細: [artifacts/eval/p4a_confidence_band_reaggregation.json](artifacts/eval/p4a_confidence_band_reaggregation.json) /
[artifacts/analysis_p4a_confidence_band.py](artifacts/analysis_p4a_confidence_band.py)
...
- **フェーズ B-3「単勝市場効率分析」Step 2 集計完了 → B-3 撤退確定**（2026-04-27）
  - 控除率 20%（3 連単 25% より 5pp 低い）の単勝で同じ favorite-longshot bias 分析を実施
  - **Step 1 完了**（2026-04-26）: `fetch_win_odds` / `load_or_download_month_win_odds` 実装、
    試行 DL 2025-12 で 4,771 / 4,771 races 取得、実勢 overround 1.36（控除率 26%、
    オッズ刻み 1.0 切り上げ等で理論 20% より +6pp 高い）
  - **12 ヶ月本番 DL 完了**（2026-04-27）: 2025-05〜2026-04 で **54,299 races / 12 ヶ月 / 0 empty**（12.9 時間、並列 10）
  - **Step 2 集計完了**（2026-04-27、`run_market_efficiency.py` を `--bet-type {trifecta,win}` に拡張、ラン 45 秒）:
    - 入力 50,483 races / 302,898 行（6 艇全揃いレースのみ集計、3,813 races 除外）
    - 古典的 favorite-longshot bias を確認（bin [0.0, 0.1) で lift=0.86、bin [0.4, 0.7) で lift > 1.06、前半 / 後半同方向 7/8 ビン）
    - **採用基準達成 = 0 / 全 82 セグメント**（全期間ビン 0/10、stadium 単軸 0/24、stadium × odds_band 0/48）
    - 最善 `ev_all_buy = 0.964`（Fukuoka stadium 単軸、CI 上限 0.988 < 1.0）、同セグメント全買いの **実 ROI = -25.87%**
  - **B-1 trifecta との比較で win が劣後**: 実勢 overround 1.36（win） vs 1.33（trifecta）/ 最大 lift 1.094 vs 1.27 / 最大 ev_all_buy 0.964 vs 0.98
  - **構造的結論**: 「控除率の低い券種なら歪みで黒字化できる」仮説は否定。単純な市場（6 通り）は参加者の予測精度が高く歪みが残りにくい
  - 詳細: [MARKET_EFFICIENCY_WIN_RESULTS.md](MARKET_EFFICIENCY_WIN_RESULTS.md) §6-§7 / [NEXT_PHASE_B3_PLAN.md](NEXT_PHASE_B3_PLAN.md) §8
- **拡張 A「複勝 (place) 市場効率分析」R1 + R2 + R3 完了 → 拡張 A 撤退確定**（2026-04-28）
  - B-3 win 撤退後の保険分析。控除率 20%（=単勝と同じ）、競艇複勝は **top-2 (1〜2 着)** が正しい仕様
  - **Step 1 + 12 ヶ月 DL 完了**（2026-04-27〜28）: `fetch_place_odds` / `load_or_download_month_place_odds` 実装、本番 54,277 races / 0 empty（12.9 時間）
  - **R1 完了**（2026-04-28）: top-2 仕様確認、`run_market_efficiency.py --bet-type place` 拡張、2025-12 単月 smoke で全 bin lift < 1.0、ev_all_buy_low 全 bin < 0.72、ただし ev_all_buy_mid が 4 bin で 1.0 超 → 楽観仮定の真偽を R2 で検証
  - **R2 完了**（2026-04-28）: `fetch_place_payouts` 実装、50 races / 100 hit boats sample で実 payout 取得 → **`pos_in_range mean = 0.245`**（実 payout は odds_low に強く偏る、median = 0.0、半数以上のレースで payout = odds_low ピッタリ）。R1 mid 楽観仮定を無効化、補正係数 `ev_corr = 0.755 × ev_low + 0.245 × ev_high` を確立
  - **R3 完了**（2026-04-28）: 補正後 ev で 12 ヶ月集計、bootstrap 2000、stadium 単軸 + stadium × odds_band 2 軸、focus 0.10-0.50
    - **採用基準達成 = 0 / 全 81 セグメント**（全期間ビン 0/9、stadium 単軸 0/24、stadium × odds_band 0/48）
    - 最善は stadium × odds_band の `5.Tamagawa | [5,10)` で `ev_corr=0.93, CI=[0.80, 1.07]`、CI 上限が初めて 1.0 を超えるが点推定 < 1.0 で確信なし
    - stadium 単軸の最善は `5.Tamagawa` で `ev_corr=0.84, CI=[0.79, 0.89]`
    - 前後半同方向 9/9 bins、期間横断で安定した overestimation 構造（点推定での偶然ではない）
  - **構造的結論**: 競艇複勝市場には odds_low 帯の系統的過大評価があるが、控除率 20% を破る規模ではない。B-1 trifecta（最善 0.98）/ B-3 win（最善 0.964）と同じ「控除率を縮める効果はあるが収支プラス化までは至らない」構造を複勝でも確認。控除率 20% の券種だから歪みやすいという仮説は否定
  - **12 ヶ月本番 payout DL（13 時間）は不実施**（採用基準達成 0 のため、R2 sample 補正で十分な判定根拠あり）
...
- **P-v race condition × odds_band 事前検証完了 → B 系列完全凍結確定**（2026-04-28）
  - 連系券種 DL 着手前の最終 sanity check として、既存 trifecta data に race condition 軸を追加して segment 集計
  - `run_market_efficiency.py` に `wind_speed_band` / `weather` group_by 軸を追加（~75 行、既存ロジック書換えなし）
  - 12 ヶ月 (2025-05〜2026-04) trifecta、focus implied_p_norm ∈ [0.10, 0.50)、bootstrap 2000
  - **採用基準達成 = 0 / 全 16 valid segments**（単軸 0/6、2 軸 0/10）
  - 最善は `wind [0,2) × odds [1,5)` で `ev=0.95, CI=[0.88, 1.02]`、CI 上限 1.02 で 1.0 を辛うじて超えるが点推定 < 1.0 で確信なし
  - 強風で歪む仮説は逆 — `[5,8)` が lift=1.01 で最も市場効率が高い（市場が風条件を織り込み済み）
  - 詳細: [MARKET_EFFICIENCY_RESULTS.md](MARKET_EFFICIENCY_RESULTS.md) §10
- **🛑 B 系列全フェーズ撤退確定（2026-04-28、Q-A 合意）**
  - 4 系統すべてで「控除率を破る歪みなし」を構造的に確認:
    - B-1 trifecta（最善 ev 0.98）/ B-3 win（最善 0.964）/ 拡張 A 複勝（最善 0.93）/ P-v condition（最善 0.95）
  - 連系券種 DL（C/C_R1）も**着手しない**: 控除率 23〜25% で大差なく、組合せ数も中間（15〜30）のため上記構造的結論を覆す根拠が薄い
  - フェーズ 3 凍結（`/inner-loop`） + フェーズ 6 完全撤退（`/model-loop`） + フェーズ 7 撤退（B-1 trifecta） + フェーズ 8 撤退（B-3 win） + 拡張 A 撤退（B-3 place） + **P-v 撤退（race condition）**
  - 実運用は引き続き停止、`run_market_efficiency.py` の改善ループは完全停止
  - 次候補は LLM 予想 (P 系列) の今後を別途議論: [NEXT_SESSION_PROMPT_LLM_DIRECTION.md](NEXT_SESSION_PROMPT_LLM_DIRECTION.md)
  - [NEXT_SESSION_PROMPT_C.md](NEXT_SESSION_PROMPT_C.md) / [NEXT_SESSION_PROMPT_C_R1.md](NEXT_SESSION_PROMPT_C_R1.md) は**凍結時の参照のみ、新規着手は不可**
- **新フェーズ「Claude Code 競艇予想システム」P1 + P2 + P3 + P3.5 完了**（2026-04-27）
  - 方針転換: 全 ML 改善ループ撤退後、判断アルゴリズムごと **Claude (LLM) に置き換える**
  - **Anthropic API 不使用**（Max プラン内 Claude Code 対話セッションで完結）
  - P1 スコープ = `/prep-races` スキル + レースカード生成基盤
    - 実装: [ml/src/predict_llm/](ml/src/predict_llm/)（`stadium_resolver` / `program_parser` /
      `history_summarizer` / `race_card_builder`）+
      [ml/src/scripts/build_race_cards.py](ml/src/scripts/build_race_cards.py) +
      [.claude/commands/prep-races.md](.claude/commands/prep-races.md)
    - 動作確認済み（4/25 桐生 / 4/27 桐生休場 + 平和島 / 2025-12-01 桐生 + 平和島休場 + 多摩川）。
      1 レース MD ~7 KB
  - P2 スコープ = `/predict` スキル（直前情報追加 + Claude が予想 JSON 出力）
    - 実装: `predict_llm/` に
      [pre_race_fetcher.py](ml/src/predict_llm/pre_race_fetcher.py) +
      [prediction_schema.py](ml/src/predict_llm/prediction_schema.py)、
      [ml/src/scripts/fetch_pre_race_info.py](ml/src/scripts/fetch_pre_race_info.py) +
      [ml/src/scripts/build_predictions_index.py](ml/src/scripts/build_predictions_index.py) +
      [.claude/commands/predict.md](.claude/commands/predict.md)
    - 既存 `collector/openapi_client.py` の `fetch_before_info` / `fetch_odds` /
      `fetch_win_odds` は流用（書き換えなし）。気象（波高・気温・水温）のみ独自スクレイプ追加
    - **過去日ドライランモード**: `data/odds/` parquet キャッシュ + K ファイル気象。展示・
      進入は欠損で OK。これで 2025-05〜2026-04 の 12 ヶ月分が再現可能
    - **当日 live モード**: boatrace.jp `beforeinfo` ページから取得（締切直前運用想定）
    - **方式 A**: Claude が race card MD を Read → 分析 → Write で JSON。CLI は直前情報
      取得 + index.json 集約のみ担当
    - **ガードレールゼロ**: 設計時の `current_odds < 1.5` 強制 skip も入れず、Claude の
      `verdict: "skip"` に完全委任
    - 設計時 `pydantic` から `dataclass + 自前 validate` に変更（依存追加回避）
  - **P3 スコープ = `/eval-predictions` スキル（予想 vs 実績の突合・ROI 算出）**
    - 実装: [ml/src/scripts/evaluate_predictions.py](ml/src/scripts/evaluate_predictions.py) +
      [.claude/commands/eval-predictions.md](.claude/commands/eval-predictions.md)
    - 過去日 = K ファイル `finish_position` + `data/odds/odds_YYYYMM.parquet` で突合、
      当日 = `fetch_race_result_full` で `trifecta_combination` + `trifecta_payout / 100`
    - **3 ステータス分類**: `settled` / `skipped_by_claude` / `no_result`。
      `no_result` は ROI / 見送り率の分母から除外
    - **集計**: 全体 ROI / 的中率（per bet, per race） / 見送り率 / 平均 confidence +
      場別 ROI + confidence 帯別 ROI（`[0.0-0.3, 0.3-0.5, 0.5-0.7, 0.7-1.0]`）
    - **payout 計算**: 過去日 = parquet オッズ × stake、当日 = `payout_yen / 100 × stake`、
      どちらも取れなければ Claude `current_odds` フォールバック（`payout_source` で記録）
    - **odds_drift_pct**: Claude `current_odds` と実 actual_odds の乖離を JSON 保存
      （集計には影響しない、後解析用）
    - **engine.py 流用しない**: 独自実装で約 500 行に収めた（既存 `_is_trio_hit` は
      3 連複用で本件不要）
    - 動作確認パス: `evaluate_predictions.py 2025-12-01 桐生` → 12 races settled,
      17 bets, 1 hit (7R `1-3-5` @ 7.2x), ROI -57.6%, hit_rate/bet 5.88%, 手計算検証 pass
  - **P3.5 スコープ = `/eval-summary` スキル（複数日の `<日付>.json` 累積集計）**
    - 実装: [ml/src/scripts/eval_summary.py](ml/src/scripts/eval_summary.py) +
      [.claude/commands/eval-summary.md](.claude/commands/eval-summary.md)
    - **入力選別**: `<日付>.json`（場フィルタなし版）のみ採用、`<日付>_<NN>.json`
      は二重計上回避のため除外（P4 で別途）
    - **引数**: `--from <YYYY-MM-DD> --to <YYYY-MM-DD>` 必須、`--month <YYYY-MM>` を
      糖衣構文として追加。単日ショートカットは P3 で済むので無し
    - **必須指標**: 期間 ROI / 月次トレンド / 場別累積 + 場×月クロス / confidence 帯別 /
      日次・月次 ROI 統計（min/median/mean/max/stddev + worst/best）/ skip_rate / avg_confidence
    - **任意指標**: bootstrap CI のみ採用（N=1000、bet 単位リサンプル、`--no-bootstrap` で off）。
      Spearman 相関 / primary_axis 別は P4 以降に後送り
    - **判定ステータス**: `production_ready` (ROI ≥ +10% & worst > -50% & CI下限 ≥ 0) /
      `breakeven` (ROI ≥ 0% & worst > -50%) / `fail`。
      `verdict.note` に「後付けフィルタチューニング厳禁」を明記（フェーズ 3〜6 教訓）
    - **既存 `evaluate_predictions.py` を書き換えない**: 出力スキーマだけ依存する独立 CLI
    - **退化テスト pass**: `--from 2025-12-01 --to 2025-12-01` → P3 `2025-12-01.json` と
      ROI / hit_rate / total_stake / total_payout / by_stadium / by_confidence_band 全一致
    - 動作確認パス: 単日（退化テスト）/ 月次（`--month 2025-12` で 30 日 missing warning）/
      存在しない期間（FileNotFoundError）/ 期間逆転（ValueError）すべて挙動確認
    - サンプル拡張は本セッション対象外。複数日サンプル生成は次フェーズで
      ユーザーが手動で `/prep-races` → `/predict` → `/eval-predictions` を回す前提
  - **次は P4 = 実走運用テスト**、別セッションで進める
  - 設計書: [LLM_PREDICT_DESIGN.md](LLM_PREDICT_DESIGN.md)
- **P4-α 完了 → P4-β 撤退クローズ → B-3 Step 2 へ移行**（2026-04-28）
  - **P4-α 結果**（2025-09〜12、桐生 1 場）: 596 bets / hit_rate 7.05% / **ROI -2.78%**、CI [-33.6%, +30.7%] で不確定領域
    - サンプルサイズ分析: 真の ROI = +10% を 0% から区別するには 9,979 bets ≈ 200 race-days 必要 → 場拡大 P4-β へ
  - **P4-β 結果**（2026-01-05〜02-18、Day 1-8、撤退クローズ）: 1,248 bet races / **62 bets** / 3 hits (4.84%) / **ROI -17.1%**
    - bootstrap 95% CI: [-100.0%, +104.5%]（半幅 ±100pp、依然不確定領域）
    - **撤退ライン (1) 通算 ROI < -15% 達成**、(2)(3) は判定不能だが構造的に bet 機会増えない（月曜通常開催の skip_rate 99-100% が支配的、Day 5+6+7+8 = 540 races / 2 bets）
  - **「Claude が EV ボーダーレースを skip しすぎ」仮説は棄却**（Step 0 シミュレーション `reeval_skip_simulation.py`）:
    - Day-ref subset (Day X 教訓引用 17 bets / 0 hits / -100%) の skip は結果的に正解
    - 機械的 EV≥1.0 採用は ROI -30.3%、MECH+BORDER 拡張も -17.3% で改善なし
    - 唯一の改善余地は「skip_rate 100% 日の 0 bet 回避」だがサンプル小で偶然の可能性
  - **撤退判定**: skip 判断は概ね合理的、機械化で ROI 改善せず、構造的に bet 機会増えない 3 点で B-3 Step 2 へ
  - **厳守事項（B-3 移行後も継続）**: predict.md / predict_llm/ / evaluate_predictions.py / eval_summary.py 書き換え禁止、後付けフィルタ追加禁止
  - **詳細計画**: [NEXT_PHASE_P4B_PLAN.md](NEXT_PHASE_P4B_PLAN.md) / P4-α 分析: [NEXT_SESSION_PROMPT_P4.md](NEXT_SESSION_PROMPT_P4.md) / P4-β 履歴: [NEXT_SESSION_PROMPT_P4B.md](NEXT_SESSION_PROMPT_P4B.md)
...
定期実行は全停止済み。本日のレース予想を手動で行いたくなった時の手順。
**自動購入はしない**こと（前提）。Claude Code 対話セッション内で完結する。

### A) 当日レース予想 (live モード、締切 30 分前運用)

```bash
# 1. 前日夜 22:00 頃: 翌日のレースカード生成（場は任意指定）
/prep-races 2026-04-29 桐生 平和島 住之江
# あるいはその日の全開催場
/prep-races 2026-04-29

# 2. 当日 各レース 30 分前: 直前情報取得 (展示・進入・直前オッズ・気象)
py -3.12 ml/src/scripts/fetch_pre_race_info.py 2026-04-29 桐生

# 3. Claude Code で予想本番（場ごとに 12R 一気が現実的）
/predict 2026-04-29 桐生
# 単発も可
/predict 2026-04-29 桐生 5

# 4. 翌朝 結果突合 (ROI / 的中率)
/eval-predictions 2026-04-29 桐生
# 全場集約版
py -3.12 ml/src/scripts/build_predictions_index.py 2026-04-29
py -3.12 ml/src/scripts/evaluate_predictions.py 2026-04-29
```

### B) 過去日ドライラン (バックテスト用、展示・進入は欠損)

```bash
# 1. レースカード生成 (program / history は data/ キャッシュから読む、必要なら DL)
/prep-races 2025-12-01 桐生

# 2. 直前情報を過去 parquet オッズ + K ファイル気象で再構成
py -3.12 ml/src/scripts/fetch_pre_race_info.py 2025-12-01 桐生

# 3. 予想 → 評価
/predict 2025-12-01 桐生
/eval-predictions 2025-12-01 桐生
```

### C) 期間集計 (複数日の累積 ROI)

```bash
py -3.12 ml/src/scripts/eval_summary.py --from 2026-04-01 --to 2026-04-29
# 月単位の糖衣構文
py -3.12 ml/src/scripts/eval_summary.py --month 2026-04
```

### 厳守事項（凍結後も継続）

- ❌ `predict.md` / `predict_llm/` / `evaluate_predictions.py` / `eval_summary.py` の書き換え禁止
- ❌ `/predict` 結果を根拠に prompt 改善 / フィルタ追加 / 場限定運用への移行禁止
- ❌ 後付けで採用基準を緩める変更禁止
- ❌ 自動購入は実施しない (dry-run / 評価のみ)
