...
> ⚠️ **注意（2026-04-24 更新）**: 以下の Session 6 / S6-3 / S6-4 の数値（ROI +794.9% 等）は
> **オッズパースのバグが修正される前のデータに基づく幻影**です。
> 修正後の実オッズで再計算した結果は 2025-09〜12 通算 ROI -21.0%（現行ルール）であり、
> 案 A（10場除外＋コース除外見直し）を適用しても 11ヶ月通算 +11.2% に留まります。
> 詳細は `BET_RULE_REVIEW_202509_202512.md` を参照してください。
> **以下の実績値・運用ルールは参照しないでください。**
...
> ⚠️ **本セクションは無効です（2026-04-24）**: オッズパースのバグ修正後は ROI +794.9% の前提が崩れており、
> 以下の月次停止基準・季節性調整・資金管理はすべて誤った前提に基づきます。適用しないでください。
> 現行の検証状況は `BET_RULE_REVIEW_202509_202512.md` を参照してください。
