...
- [x] T6-B: 「3連複のみ的中」リカバリー分析（2026-04-22 完了 → **3連複追加は非推奨と判定**）
...
## T6-B リカバリー分析 結果（2026-04-22 実施）

期間: 2025-10〜12（合成オッズ）、3連単 prob=0.07/ev=2.0 × 3連複 prob=0.20/ev=4.0

### カテゴリ別件数（race_id 結合、全11,895レース）

| カテゴリ | 件数 | 備考 |
|----------|------|------|
| 両方的中 | 23件 | 3連単的中時は高確率で3連複も的中 |
| 3連単のみ的中 | 5件 | 3連複フィルタで除外された組合せ |
| **3連複のみ的中（純リカバリー）** | **190件** | **3連単外れだが3連複は的中** |
| 両方外れ | 11,677件 | — |

### 収支比較

| 指標 | 3連単単独 | 3連単+3連複（組み合わせ） |
|------|----------|----------------------|
| 投資額 | 545,600円 | 1,393,900円 |
| 払戻額 | 973,959円 | 2,176,869円 |
| **ROI** | **+78.5%** | **+56.2%** ▼ |
| 月別ROI | 10月+X%, 11月+X%, 12月+X% | 10月+17.6%, 11月+61.7%, 12月+84.8% |

> ※ 合成オッズ使用（実オッズは3連単: 実績+1,312% S6短期、3連複: 未取得）
...
|------|-----|
| 純リカバリー件数 | 190件 / 3ヶ月 |
| 純リカバリー払戻合計 | 1,057,204円 |
| 純リカバリー投資額 | 27,100円 |
| 純リカバリーROI | +3,801% |
| 3連複的中時 avg オッズ | 56.5x（合成） |
| 3連複的中時オッズ分布 | <20x: 52件 / 20-40x: 20件 / 40-100x: 121件 / 100x+: 20件 |

### 分析・考察

**純リカバリー自体は高収益（+3,801%）だが、3連複投資コストが3連単ROIを相殺する構造。**

1. **3連複投資コストの問題**: 3連単ベット額545,600円に対し3連複848,300円を追加投資（1.56倍増）。これが組み合わせROIを+78.5% → +56.2% へ22pp悪化させる。

2. **純リカバリーの非効率性**: 190件のリカバリー的中は存在するが、8,483点の3連複ベットに対してわずか2.24%。的中しない7,756点（775,600円）が純損失。

3. **3連単と3連複の相関**: 両方的中23件のみ（3連単的中28件のうち82%は3連複も的中）。相関が強く、3連複は3連単の補完にならず重複投資に近い。

4. **合成オッズの限界**: 3連複は実オッズ取得が未実装のため合成オッズで試算。実オッズではROIがさらに変動する可能性あり（ただしリカバリー件数・相関構造は変わらない）。

### 結論：**3連複の追加購入は非推奨**

3連単単独の高ROI構造（高オッズ×絞り込み）を3連複が稀釈する。現行の3連単専念戦略を継続することを推奨。

純リカバリー190件の存在は「3連複専用フィルタ」（3連単と全く異なる独立予算）として切り出せば単体でROI+3,801%だが、実際の実オッズ取得・システム実装コストを考慮すると優先度は低い。
