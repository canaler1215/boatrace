...
- [ ] **0-2. `docker-compose.yml` 作成**
  - PostgreSQL 16 サービス定義、ボリューム永続化
  - 完了条件: `docker compose up -d` で DB が起動し、マイグレーションが通る
- [ ] **0-3. ローカルセットアップ手順書（README 追記 or `SETUP.md`）**
  - 手順: clone → `.env` コピー → `docker compose up` → `pip install -r ml/requirements.txt` → マイグレーション → smoke test
  - Windows での `lhasa` 代替（WSL 推奨）を明記
- [ ] **0-4. smoke test スクリプト**
  - 1日分のデータで `run_backtest.py` を通す最小ケース
  - 完了条件: CI・ローカル両方で 5 分以内に完走
