...
最終更新: 2026-04-27（フェーズ 8 B-3 拡張 A 複勝 Step 1 完了 — `fetch_place_odds` / `load_or_download_month_place_odds` 実装、2025-12 試行 DL で **4,749 / 4,771 races** 取得（99.5%、22 empty は江戸川 12-09 集中）、所要 75 分。複勝の overround は range 構造で単純解釈不能（`sum(1/odds_low)=3.09` が理論 3 に最も近い）→ Step 2 で実 ROI ベース必須。詳細は [MARKET_EFFICIENCY_PLACE_RESULTS.md](MARKET_EFFICIENCY_PLACE_RESULTS.md)。なおフェーズ 7 / B-1 / B-3 win 完全撤退確定状態は維持。**全フェーズ撤退状態（案 A 完全凍結）+ B-3 win Step 2 撤退 + B-3 拡張 A Step 1 完了**）
...
### 拡張 A: 複勝 (place) — Step 1 完了 / Step 2 待機（2026-04-27）

| Step | 内容 | 状態 | コスト |
|---|---|---|---|
| 1 | `fetch_place_odds` 実装 + 1 ヶ月試行 DL | **[x] 完了** 2026-04-27（4,749/4,771 races, 99.5%） | 実装 1h + DL 75 分 |
| 2-DL | 残り 11 ヶ月本番 DL（バックグラウンド） | **[ ] 未着手**（ユーザー承認待ち） | DL ~15h |
