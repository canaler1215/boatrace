...
- [ ] **6-8. 本番 10 trial 連続実行**（2026-04-24 改訂で 8→10）
  - 前提: `data/odds/2025-05〜2026-04` の 12 ヶ月分実オッズが揃っていること
  - 2026-04-24 時点で `download_odds.py` をローカル別ウィンドウで直列実行中（所要 15〜18 時間）
  - ダウンロード完了後 `py -3.12 ml/src/scripts/run_model_loop.py` で連続実行
  - 10 trial 構成は T00〜T09（旧 T06 を T06_feature_subsample に差し替え、
    T08/T09 として baseline seed 反復を追加）。詳細は [trials/README.md](trials/README.md) 参照
  - 結果を `MODEL_LOOP_RESULTS.md`（新規）にまとめる
- [ ] **6-9. 次イテレーション設計（必要に応じて）**
  - `primary_score` 上位 trial の近傍を 2〜3 本追加
  - verdict=pass が出るか、15 trial 到達まで継続（2026-04-24 改訂）
  - 10 trial pass 0 の時点で pass 事後確率を β(1,1) 事前で更新、
    P(p>10%)>20% なら追加 5 trial、そうでなければ構造変更提案フェーズへ
  - 構造変更先は LambdaRank 単独指名ではなく、特徴量拡張 / 目的関数変更 /
    キャリブレーション再設計 / Purged CV のツリーから PoC で選ぶ
    （MODEL_LOOP_PLAN §5 撤退条件を参照）
...
| 6. モデル構造ループ | 7 / 9 | 🔴 **最優先**（6-8, 6-9 は本番オッズ DL 完了待ち） |
| **合計** | **22 / 30** | |
