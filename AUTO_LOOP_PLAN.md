...
- [ ] **6-9. 次イテレーション: T10〜T12（T07 近傍確証）** 🔴 進行中
  - T10_window_2024_weight_seed1 — T07 と同設定 + `lgb_params.seed=1`（seed 耐性検証）
  - T11_window_2024_weight_seed2 — T07 と同設定 + `lgb_params.seed=2`（同上もう 1 本）
  - T12_window_2024_weight_strong — T07 ベースで `recency_months=3, recency_weight=3.0`（直近強調の感度）
  - T10/T11 で T07 が再現すれば本採用候補、再現しなければ「T07 は seed ガチャ」と確定して構造変更フェーズへ
- [ ] **6-10. 構造変更フェーズ（撤退条件発動時）**
  - T12 まで回して verdict=pass 再現 0 もしくは 15 trial 到達 → 構造変更提案フェーズへ
  - 10/15 trial pass 0 の時点で pass 事後確率を β(1,1) 事前で更新、
    P(p>10%)>20% なら追加 5 trial、そうでなければ構造変更提案
...
| 6. モデル構造ループ | 8 / 10 | 🔴 **最優先**（6-8 完了、6-9 進行中、6-10 は条件付き） |
| **合計** | **23 / 31** | |
