...
- [ ] **6-10. 構造変更フェーズ（パラメータ探索打ち止め → 構造変更）** 🔴 進行中
  ...
  - **6-10-c. キャリブレーション再設計 / Purged CV** 📋 未着手（次セッション着手予定、2026-04-25 案 Y 確定）
    - 案 Y 確定（ユーザー承認 2026-04-25）: R1 (LambdaRank) は Walk-Forward へ進めず、構造変更ツリー次候補へ移行
    - 候補 1: キャリブレーション再設計（per-class IR → 結合 IR / Dirichlet）
    - 候補 2: Purged/Embargoed time-series CV
    - 次セッションプロンプト: [NEXT_SESSION_PROMPT.md](NEXT_SESSION_PROMPT.md)
