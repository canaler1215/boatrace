...
最終更新: 2026-04-26（🛑 フェーズ 7 / B-1 完全撤退確定。Step 2 単軸 21 + 2 軸 41 = 全 62 サブセグメントすべて採用基準未達（最高 ev=0.98 / 蒲郡 × 1 コース）。**全フェーズ撤退状態（案 A 完全凍結）へ移行**。次フェーズ B-3「単勝市場効率分析」を着手予定として保留。詳細は [NEXT_PHASE_B1_PLAN.md](NEXT_PHASE_B1_PLAN.md) §9 / [NEXT_PHASE_B3_PLAN.md](NEXT_PHASE_B3_PLAN.md)）
...
## フェーズ 8（予定）: 単勝市場効率分析（B-3）

**着手予定（2026-04-26 ユーザー合意、別セッションで進める）**: フェーズ B-1 撤退後の次候補として
**B-3「馬券種転換による市場効率分析（単勝）」** を保留する。
...
| Step | 内容 | コスト |
|---|---|---|
| 1 | `fetch_win_odds` 実装 + 1 ヶ月試行 DL で API 動作確認 | 5〜10 分 |
| 2 | 12 ヶ月本番 DL（バックグラウンド）+ `--bet-type win` での市場効率分析 | DL 12〜24 時間 + 分析 数時間 |
| 3 | サブセグメント分析（歪みが見つかった場合のみ） | 半日 |
| 4 | 単勝戦略バックテスト（採用基準達成時のみ） | 1〜2 日 |
| 5 | 撤退判定 or 拡張 | 数時間 |

### 着手前の判断ポイント

- **新規 DL 必須**: 単勝オッズは既存 `data/odds/` に無い（trifecta / trio のみ）
- 既存 `openapi_client.fetch_odds` の API スキーマで単勝相当が取れるかは未確認 → Step 1 の試行 DL で判定
- 取れなければ B-3 撤退、案 A 完全凍結を継続
