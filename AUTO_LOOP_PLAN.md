...
最終更新: 2026-04-26（フェーズ 8 B-3 Step 1 完了 — 単勝オッズ DL 動作確認 OK、`fetch_win_odds` / `load_or_download_month_win_odds` 実装、2025-12 試行 DL で 4,771 / 4,771 races 取得（実勢 overround 1.36 = 控除率 26%）。Step 2（残り 11 ヶ月本番 DL ≒ 14h + `--bet-type win` 拡張）はユーザー承認待ち。詳細は [MARKET_EFFICIENCY_WIN_RESULTS.md](MARKET_EFFICIENCY_WIN_RESULTS.md)。なおフェーズ 7 / B-1 完全撤退確定状態は維持（最高 ev=0.98 / 蒲郡 × 1 コース）。**全フェーズ撤退状態（案 A 完全凍結）+ B-3 Step 1 完了**）
...
## フェーズ 7: 市場効率の歪み分析（B-1） — 🛑 完全撤退確定

**完全撤退確定（2026-04-26）**: Step 1〜2（単軸 + 2 軸組合せ）で **採用基準達成 0 / 全 62 サブセグメント**。
[NEXT_PHASE_B1_PLAN.md](NEXT_PHASE_B1_PLAN.md) §8 の撤退条件「Step 2 でセグメントを細分しても有意な歪みが見つからない」に該当。**案 A 完全凍結状態へ移行**。
...
### Step 1 結果（2025-05〜2026-04, 12 ヶ月、646 万 combo / 54,154 レース）

| 暗黙帯 (norm) | n | lift | ev_all_buy | 90% boot CI | 採用基準 |
|---|---:|---:|---:|---|:---:|
| [0.0001, 0.0003) | 321,211 | 0.436 | 0.347 | [0.309, 0.564] | ✅ overpriced |
| [0.0003, 0.001) | 1,381,309 | 0.621 | 0.519 | [0.576, 0.664] | ✅ overpriced |
| [0.10, 0.20) | 37,509 | **1.101** | 0.845 | [1.080, 1.122] | ✅ underpriced |
| [0.20, 0.50) | 1,123 | **1.199** | 0.901 | [1.103, 1.293] | ✅ underpriced |

- **採用基準該当: 4 ビン / 9 ビン**（n ≥ 1,000、|lift-1| ≥ 0.10/0.15、CI excl. 1）
- **前半 6 ヶ月 / 後半 6 ヶ月で全 9 ビン同方向**（一過性ではない）
- 古典的 favorite-longshot bias の競艇版を統計的に確認

### ⚠️ Step 1 警告

`ev_all_buy` が全ビンで 1.0 未満（最大 0.901）。**lift > 1.10 でも控除率の壁は破れていない**。
ビン内全買いでは依然 -10〜-15% の損失。Step 3 採用基準（通算 ROI ≥ +10%）は
Step 1 結果から直接予測できる範囲では達成不能。サブセグメント分析（Step 2）で
`ev_all_buy > 1.0` となる範囲が存在するかを確認する必要がある。

### Step 2 結果（2026-04-26、単軸 4 軸 / focus implied 0.10〜0.50）

| 軸 | n≥1000 サブセグメント | ev_all_buy 最高 | ev CI 上限 | 採用 |
|---|---:|---:|---:|:---:|
| stadium | 21 | 0.96 (7.Gamagori) | 1.03 | 0 |
| course  | 2 | 0.86 (1コース) | 0.87 | 0 |
| odds_band | 2 | 0.87 ([1,5)) | 0.90 | 0 |
| month | 12 | 0.89 (2026-04) | 0.96 | 0 |

**全 21 サブセグメントで `ev_all_buy < 1.0`**。最有望は蒲郡 (lift=1.24) だが控除率の壁を破れず。

### Step 2 単軸の判定
B-1 撤退候補（2 軸組合せ未確認時点）。

### Step 2 / 2 軸組合せ結果（2026-04-26、ユーザー指示で実施）

| ペア | 全 cell | n≥1000 | ev_all_buy 最高 | ev CI 上限 |
|---|---:|---:|---:|---:|
| stadium × odds_band | 48 | 20 | 0.94 (蒲郡 \| [5,10)) | 1.03 |
| **stadium × course** | 127 | 21 | **0.98 (蒲郡 \| 1コース)** | 1.06 |
| stadium × month     | 278 | 0 | — | — |

**全 41 サブセグメント（n≥1000）で採用基準未達**。最有望でも `ev=0.98, CI=[0.90, 1.06]` で控除率破壊の確信なし。

### 最終判定: 🛑 B-1 撤退確定（2026-04-26 ユーザー合意）

- 単軸 21 + 2 軸 41 = **計 62 サブセグメント全敗**
- 競艇 3 連単市場の favorite-longshot bias は「控除率を縮める効果」止まり
- 全フェーズ撤退状態（フェーズ 3 凍結 + フェーズ 6 完全撤退 + フェーズ 7 撤退）へ移行
- 詳細: [NEXT_PHASE_B1_PLAN.md](NEXT_PHASE_B1_PLAN.md) §9「B-1 撤退結果」

### 成果物

- [ml/src/scripts/run_market_efficiency.py](ml/src/scripts/run_market_efficiency.py) — Step 1〜2 スクリプト（~680 行）
- [artifacts/market_efficiency_2025-05_2026-04_trifecta.csv](artifacts/market_efficiency_2025-05_2026-04_trifecta.csv) — Step 1 全期間ビン別集計
- [artifacts/market_efficiency_2025-05_2025-10_trifecta.csv](artifacts/market_efficiency_2025-05_2025-10_trifecta.csv) — Step 1 前半 6 ヶ月
- [artifacts/market_efficiency_2025-11_2026-04_trifecta.csv](artifacts/market_efficiency_2025-11_2026-04_trifecta.csv) — Step 1 後半 6 ヶ月
- [artifacts/market_efficiency_2025-05_2026-04_trifecta.png](artifacts/market_efficiency_2025-05_2026-04_trifecta.png) — キャリブレーションプロット
- [artifacts/market_efficiency_segment_stadium_focus_0.10-0.50_2025-05_2026-04_trifecta.csv](artifacts/market_efficiency_segment_stadium_focus_0.10-0.50_2025-05_2026-04_trifecta.csv) — Step 2 場別
- [artifacts/market_efficiency_segment_course_focus_0.10-0.50_2025-05_2026-04_trifecta.csv](artifacts/market_efficiency_segment_course_focus_0.10-0.50_2025-05_2026-04_trifecta.csv) — Step 2 コース別
- [artifacts/market_efficiency_segment_odds_band_focus_0.10-0.50_2025-05_2026-04_trifecta.csv](artifacts/market_efficiency_segment_odds_band_focus_0.10-0.50_2025-05_2026-04_trifecta.csv) — Step 2 オッズ帯別
- [artifacts/market_efficiency_segment_month_focus_0.10-0.50_2025-05_2026-04_trifecta.csv](artifacts/market_efficiency_segment_month_focus_0.10-0.50_2025-05_2026-04_trifecta.csv) — Step 2 月別
- [MARKET_EFFICIENCY_RESULTS.md](MARKET_EFFICIENCY_RESULTS.md) — Step 1 結果ドキュメント
- [MARKET_EFFICIENCY_SEGMENT_RESULTS.md](MARKET_EFFICIENCY_SEGMENT_RESULTS.md) — Step 2 結果ドキュメント、案 A〜C 提案

### 関連ドキュメント

- 詳細計画: [NEXT_PHASE_B1_PLAN.md](NEXT_PHASE_B1_PLAN.md)
- Step 1 結果: [MARKET_EFFICIENCY_RESULTS.md](MARKET_EFFICIENCY_RESULTS.md)
- Step 2 結果: [MARKET_EFFICIENCY_SEGMENT_RESULTS.md](MARKET_EFFICIENCY_SEGMENT_RESULTS.md)
- 次フェーズ計画: [NEXT_PHASE_B3_PLAN.md](NEXT_PHASE_B3_PLAN.md)

## フェーズ 8: 単勝市場効率分析（B-3）

**Step 1 完了（2026-04-26）、Step 2 ユーザー承認待ち**: フェーズ B-1 撤退後の方針転換として
...
| 2 | 残り 11 ヶ月本番 DL（バックグラウンド）+ `--bet-type win` での市場効率分析 | [!] ユーザー承認待ち | DL ≒ 14h + 分析 数時間 |
...
- **残り 11 ヶ月本番 DL** (2025-05〜2025-11 + 2026-01〜2026-04): 14 時間バックグラウンド推奨
- **DL 実行可否はユーザー承認後**
- DL 完了後は `run_market_efficiency.py --bet-type win` の拡張が必要
- 詳細計画: [NEXT_PHASE_B3_PLAN.md](NEXT_PHASE_B3_PLAN.md)
- Step 1 結果: [MARKET_EFFICIENCY_WIN_RESULTS.md](MARKET_EFFICIENCY_WIN_RESULTS.md)
