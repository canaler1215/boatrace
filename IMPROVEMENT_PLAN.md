...
**最終更新**: 2026-04-18
**現在のセッション**: **Session 6 完了（S6-4 運用ルール策定済み）→ Session 7 実運用開始・モデル改善へ**
...
| `CLAUDE.md` / `IMPROVEMENT_PLAN.md` | S6-4 運用ルール策定・ドキュメント更新（2026-04-18） |
...
- [x] S6-3: Walk-Forward 期間延長（2024-01〜2025-12） — **2026-04-18 完了**
  - 24ヶ月検証で全月プラス（ROI +794.9%）を確認、統計的信頼性が確立
  - Session 5 ルール（prob≥7%, EV≥2.0 等）を維持
  - 結果: `artifacts/Session6-2/walkforward_long_merged.csv`
- [x] S6-4: 運用ルール策定（S5-4 の移管） — **2026-04-18 完了**
  - Walk-Forward 長期結果を踏まえた実運用判断基準の文書化
  - 月次許容損失・停止条件・モデル再学習タイミングの定義
  - 季節性リスク（3月・6月は低ROI傾向）を考慮したベット額調整ガイド

### Session 7: 実運用開始・モデル改善

**目標**: 統計的に検証済みの戦略を実運用に移し、並行してモデルの1着識別能力改善に取り組む

- [ ] S7-1: 実運用開始（少額テスト運用）
  - 月次予算: 30,000〜50,000円（300〜500点/月相当）
  - 運用ルール S6-4 に従いモニタリング（月次ROI確認・停止条件の遵守）
  - 3ヶ月後に継続・拡大・停止を判断
- [ ] S7-2: 1着識別能力の根本改善（モデルアーキテクチャ変更）
  - Pairwise 比較モデル（Bradley-Terry）または Listwise ランキング学習（LambdaMART）への移行検討
  - 前提: 全ビン~17% の構造バイアスが現行 LightGBM multiclass の限界と確認済み
  - 実装: `ml/src/model/` に新モデルクラスを追加、`run_retrain.py` に切り替えオプション
- [ ] S7-3: 新特徴量追加（識別能力の底上げ）
  - レーサー間対戦成績（同一レースでの過去成績）
  - コース×場の交差勝率（直近3ヶ月）
  - 当日展示タイム×モーター勝率の交互作用特徴量

---

## Session 6 GH Actions 分析結果（2026-04-17）

### 実行結果ファイル（artifacts/Session6/）

| ファイル | 内容 |
|---------|------|
| `model_metrics.txt` | version=202604, rps=0.1565, top1_accuracy=31.45%, samples=1,403,845 |
| `model_latest.pkl` | softmax_calibrators=list[IsotonicRegression×6] |
| `calibration_202512_summary.csv` | 1착 ECE raw=0.13561, calibrated=0.13371, trifecta ECE=0.001327 |
| `walkforward_202510-202512.csv` | ベット7,023件、ROI +1,312%、的中140件 |

### Walk-Forward 結果（Session 6）
...
的中率/bet: 1.99% | avg matched_odds: 708x | avg top_prob: 0.0848

### Session 6 キャリブレーション分析結果

| クラス | S5 ECE（raw） | S6 ECE（raw） | S6 ECE（calibrated） | 評価 |
|--------|-------------|-------------|---------------------|------|
| 1着 | 0.13720 | 0.13561 | **0.13371** | 微改善 |
| 2着 | 0.05629 | 0.05452 | 0.05530 | 微改善 |
| 3着 | 0.03700 | 0.03245 | 0.03520 | raw改善 |
| 4着 | 0.02796 | 0.03264 | 0.03140 | 微悪化 |
| 5着 | 0.06277 | 0.05865 | 0.05885 | 微改善 |
| 6着 | 0.08750 | 0.09115 | 0.09624 | **悪化** |
| trifecta | 0.001123 | **0.001327** | — | **▼ +18%悪化** |

1着キャリブレーションビン別（S6、構造バイアス継続）:

| ビン | 予測確率 | 実際的中率 | 乖離 |
|------|---------|----------|------|
| 0-10% | 5.0% | 16.7% | 過小推定 3.3x |
| 10-20% | 13.7% | 16.6% | ほぼ適正 |
| 20-30% | 24.5% | 16.1% | 過大推定 |
| 30-40% | 34.5% | 16.9% | 過大推定 2.0x |
| 50-60% | 55.0% | 18.3% | 過大推定 3.0x |
| 70-80% | 73.5% | 17.1% | 過大推定 4.3x |
...
| S5 | prob≥7%, EV≥2.0, フィルタあり | 6,967 | +1,044% | 1.87% | 613x | 0.13720 |
| **S6** | **prob≥7%, EV≥2.0, フィルタあり** | **7,023** | **+1,312%** | **1.99%** | **708x** | **0.13561** |

月次ROI安定性:
- S5: 611%〜1,575%（幅 964pp）
- **S6: 1,157%〜1,485%（幅 328pp）← 大幅安定化**

### Session 6 重大発見

**1着の実際的中率が全予測ビンで均一に ~16〜17%（ランダム ≈ 1/6）**:
- モデルが「誰が1着か」をほとんど識別できていない
- 予測 5% でも 80% でも、実際の的中率は同じ ~17%
- ROI プラスの源泉は確率精度ではなく、**高オッズ × 低ベット数の宝くじ構造**
- 的中時 avg odds が 708x と高いのはこの構造の表れ

**trifecta ECE 悪化の継続**:
- Softmax 正規化 + IR でも 10-20% ビンの過大推定（6.9x）は解消せず
- 0-10% ビンは適正（1.1x）だが 10%+ ビンで急激に悪化する構造は S5 と同様

### Session 6 → 次ステップ移行の判断

| 観点 | 評価 | 判断 |
|------|------|------|
| ROI改善 | +268pp（+1,044%→+1,312%） | 継続価値あり |
| 月次安定性 | 幅が 964pp→328pp に大幅改善 | 統計的信頼性が向上 |
| trifecta ECE | +18% 悪化（期待外れ） | Plackett-Luce 自体に限界 |
| 1着識別能力 | 全ビン ~17% で識別なし | モデルアーキテクチャの根本課題 |

**→ S6-3（長期 Walk-Forward 2024-01〜2025-12）完了: 全24ヶ月プラス、ROI +794.9%**

### Session 6 S6-3 長期 Walk-Forward 結果（2026-04-18、artifacts/Session6-2/）

#### 全体サマリー（2024-01〜2025-12、24ヶ月）
...
月別統計: 平均ROI 796.5%、中央値 795.6%、標準偏差 220.4%、最低 +297.9%（2024-06）、最高 +1,430.3%（2025-12）

季節性: 3月（平均465%）・6月（578%）低め、7〜9月（~900%）・12月（1,106%）高め
...
| 観点 | 評価 |
|------|------|
| 統計的信頼性 | **確立**（24/24ヶ月プラス、偶然では説明不可） |
| ROI水準 | +794.9%（短期+1,312%より低いが長期安定） |
| 月次安定性 | std 220%（短期S5の~489%から大幅改善） |
| 宝くじ構造 | 継続（avg odds 586x × 的中率1.53%） |
| 1着識別能力 | 未解決（全ビン~17%の構造バイアス継続） |

**→ 戦略として機能することが統計的に確認された。S6-4 運用ルール策定完了。**

### Session 6 S6-4 運用ルール（2026-04-18 策定）

S6-3 長期 Walk-Forward 結果（24ヶ月全月プラス、ROI +794.9%）に基づく実運用判断基準。
...
#### 2. 月次モニタリング基準（S6-3 24ヶ月データ準拠）

| ゾーン | ROI基準 | 24ヶ月実績 | アクション |
|--------|---------|-----------|---------|
| 正常域 | ≥ 500% | 22/24ヶ月（92%） | 継続 |
| 注意域 | 300%〜499% | 1/24ヶ月（4%） | 要観察（翌月改善なら継続） |
| 警戒域 | 0%〜299% | 0/24ヶ月（史上最低+297.9%に隣接） | **一時停止** → 再学習 + 直近分析 |
| 危険域 | < 0% | 0/24ヶ月（未発生） | **即時停止** → モデル・ルール総点検 |

#### 3. 停止・見直し条件

| 条件 | アクション |
|------|----------|
| 月次ROI < 0% | **即時停止** → `run_calibration.py` + `run_segment_analysis.py` で原因分析 |
| 月次ROI < 300% が2ヶ月連続 | **一時停止** → `run_retrain.py` 再学習 + 直近3ヶ月セグメント分析 |
| 月次ROI < 500% が3ヶ月連続 | **ルール見直し** → グリッドサーチで閾値再最適化 |
| ベット数が月平均（2,600件）の50%未満 | **警告** → データ取得・フィルタ設定を確認 |
...
| 定期再学習 | 月1回（毎月初旬） |
| 緊急再学習 | 警戒域・危険域トリガー時 |
| コマンド | `python ml/src/scripts/run_retrain.py` |

#### 5. 季節性リスクへの対応（S6-3 実績ベース）

| 月 | 実績平均ROI | 推奨ベット額 |
|----|-----------|------------|
| 3月 | 465%（年間最低） | **70円/点**（-30%） |
| 6月 | 578%（年間2位低） | **80円/点**（-20%） |
| 7〜9月 | ~900% | 100円/点（標準） |
| 12月 | 1,106%（年間最高） | 100円/点（標準） |
| 他月 | ~800% | 100円/点（標準） |

代替: `--kelly-fraction 0.25` で確率ベース自動調整

#### 6. 資金管理ガイド

| 指標 | 推奨値 | 根拠 |
|------|--------|------|
| 推奨初期資本 | ≥ 200,000円 | 月次投資額（~260,000円）に対する余裕 |
| 月次投資額の目安 | ~260,000円 | 月平均ベット数2,600件 × 100円 |
| 歴史的最悪月 | +297.9%（2024-06） | 最悪でも大幅プラス（過去実績） |
| 連続損失リスク | 0/24ヶ月 | 統計的に非常に低い |

**注意**: 過去実績は将来の成績を保証しない。宝くじ構造（的中率1.53%、avg odds 586x）のため月次ベット数が少ないと確率的ブレが大きくなる。

#### 7. 日次・月次オペレーション手順

```bash
# [日次] 本番予測（当日レースの購入候補抽出）
...
# [半期] Walk-Forward 再検証（直近6ヶ月）
python ml/src/scripts/run_walkforward.py \
  --start YYYY-MM --end YYYY-MM --retrain --real-odds \
  --prob-threshold 0.07 --ev-threshold 2.0 \
  --exclude-courses 2 4 5 --min-odds 100 --exclude-stadiums 11
```
...
1. **1着識別能力の欠如**: 全予測ビンで実際的中率 ~17%（ランダム1/6と同等）。ROIは確率精度ではなく高オッズ×絞り込みに依存
2. **宝くじ構造リスク**: 的中率1.53%・avg odds 586xのため、月次ベット数が少ないと確率的ブレが大きい
3. **市場変化リスク**: 24ヶ月の統計的信頼性はあるが、競艇規則変更・選手構成変化には適応できない
4. **trifecta ECE**: 3連単の確率精度（ECE 0.001327）は依然高め。EV・確率フィルタで代替的に制御
